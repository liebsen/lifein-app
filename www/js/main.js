...
$(document).on('submit','#preferencias',function(e){
    ...
    data.geo = { lat: $('#direccion').attr('lat')||0, lng : $('#direccion').attr('lng')||0 },
    data.layout = layout;
...
$(document).on('click','.login',function(){
    ...
    $(this).prop('disabled',true).addClass('disabled').animate({opacity:0.7}).text("Por favor espere ... ");
...
$(document).on('click','#panico_ok',function(e){
  var _key = $(this).attr('key');
  firebase.database().ref('/panico/'+key+'/'+_key+'/aprobado').set(1);
